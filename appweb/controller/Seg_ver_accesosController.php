...
require_once (ROOT_DIR . "/model/Seg_ver_accesosModel.php");
...
switch ($method) {
 ...
 function  filterId($input ){
    $tseg_ver_accesos = new Seg_ver_accesosModel();
$p_id = !empty($input['id'])?$input['id']:$_GET['id'];;

    $var = $tseg_ver_accesos->findId($p_id);
...
function  filterPaginateAll($input){
    ...
    $tseg_ver_accesos = new Seg_ver_accesosModel();
    $var = $tseg_ver_accesos->findpaginateall($filter, $p_limit, $p_offset);
    echo json_encode($var);
}

function  filterAll($input){
    $tseg_ver_accesos = new Seg_ver_accesosModel();
    $var = $tseg_ver_accesos->findall();
    echo json_encode($var);
}
...
$p_fh_mod = !empty($input['fh_mod'])?$input['fh_mod']:$_POST['fh_mod'];;
    ...
    $tseg_ver_accesos = new Seg_ver_accesosModel();
	$var = $tseg_ver_accesos->insert($p_id,$p_cod_usu,$p_cod_apl,$p_estado,$p_usu_cre,$p_fh_cre,$p_usu_mod,$p_fh_mod);
...
$p_fh_mod = !empty($input['fh_mod'])?$input['fh_mod']:$_POST['fh_mod'];;
    ...
    $tseg_ver_accesos = new Seg_ver_accesosModel();
	$var = $tseg_ver_accesos->update($p_id,$p_cod_usu,$p_cod_apl,$p_estado,$p_usu_cre,$p_fh_cre,$p_usu_mod,$p_fh_mod);
...
$p_id = !empty($input['id'])?$input['id']:$_POST['id'];;
    ...
    $tseg_ver_accesos = new Seg_ver_accesosModel();
	$var = $tseg_ver_accesos->delete($p_id);
